...
## Privacy at a glance (see `../policies/PRIVACY.md`)
...
- Cloud LLM: when enabled, the question text and retrieved artwork notes are
  ...
  `../policies/CLOUD_LLM_DISCLOSURE.md`). In dev/mock mode nothing leaves the
  laptop.
...
- For museum-specific facts, add or improve the content pack when a detail is
  missing. Broader questions may be answered from Gemini general knowledge and
  should not be treated as museum-verified evidence.
